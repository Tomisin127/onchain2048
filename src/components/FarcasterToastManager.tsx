...
export default function FarcasterToastManager({ children }: FarcasterToastManagerProps): JSX.Element {
  ...
  const handleManifestSuccess = (result: ManifestResult): void => {
    ...
    toast.success('Manifest Signed Successfully!', {
      description: `Domain: ${window.location.hostname}`,
      duration: 3000,
    })
          ...
          <div className="text-xs space-y-3 text-foreground">
            ...
            <div className="flex gap-2 justify-center">
              <button
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground px-3 py-2 rounded text-sm font-medium transition-colors"
              >
                Copy Again
              </button>
            </div>
