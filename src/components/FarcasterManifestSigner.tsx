...
interface FarcasterManifestSignerProps {
  domain?: string
...
export default function FarcasterManifestSigner({ 
  ...
  onSuccess,
  onError
}: FarcasterManifestSignerProps): JSX.Element | null {
  ...
  // Get the domain from the current window location if not provided
  const effectiveDomain = domain || (typeof window !== 'undefined' ? window.location.hostname : '')
  ...
  }, [isAlreadySigned, isCheckingSignedStatus, effectiveDomain])
